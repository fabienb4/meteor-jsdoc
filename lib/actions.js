...
Actions.prototype.build = function() {
  ...
  var cmd = `find . -type f ! -path "*/.git/*" ! -path "*/.meteor/*" | xargs grep -lswH "@summary" | xargs -L 10000 -t ${self.config.nodePath} ${jsdocPath} ${jsdocArgs} -t ${jsdocTmplPath} -c ${jsdocConf} -d ${self.config.docsPath} 2>&1`;
